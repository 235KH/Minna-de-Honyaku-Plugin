...
(function() {
    ...
    function extractCoreTitle(title) {
        // 预处理: 使用正则寻找包含中文、日语字符或英文字母的括号，并移除
        title = title.replace(/\s+/g, '');

        let bracketMatch = title.match(/[\(\[\{][^\)\]\}]*[\u4e00-\u9faf\u3040-\u30ffa-zA-Z]+[^\)\]\}]*[\)\]\}]/);
        while (bracketMatch) {
            // 移除该括号内容
            title = title.replace(bracketMatch[0], '');
            // 再次尝试匹配，查看是否还有其他符合条件的括号
            bracketMatch = title.match(/[\(\[\{][^\)\]\}]*[\u4e00-\u9faf\u3040-\u30ffa-zA-Z]+[^\)\]\}]*[\)\]\}]/);
        }
    
        // 进行主匹配
        let match = title.match(/^(.*?[\u4e00-\u9faf\u3040-\u30ff|\)\]\}]+)/);
    
        // 如果没有匹配到中文或日语字符，尝试匹配连续的英文字符和空格
        if (!match) {
            match = title.match(/^[a-zA-Z\s]+/);
        }
                    ...
                    setTimeout(function() {
                        ...
                        if (confirmButton) {
                            ...
                            if (fullAutoCheckboxState) {
                                ...
                                }, 500);  // 在所有其他操作完成后稍微等待一下再返回，这里是0.5秒后
                            }
                        } else {
